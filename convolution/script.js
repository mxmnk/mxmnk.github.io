...
var filterlist = [
  ...
  {
    label: 'Box Blur',
    value: [1, 1, 1, 1, 1, 1, 1, 1, 1].map((i) => (i * 1) / 9),
  },
  {
    label: 'Gaussian Blur',
    value: [1, 2, 1, 2, 4, 2, 1, 2, 1].map((i) => 1 / 16),
  },
  {
    label: 'Gaussian Blur 2',
    value: [ 1, 4, 6, 4, 1, 4, 16, 24, 16, 4, 6, 24, -476, 24, 6, 4, 16, 24, 16, 4, 1, 4, 6, 4, 1]
      .map((i) => (i / 256) * -1),
  },
  ...
  { label: 'Vertical Line 2', value: [-1, 0, 1, -2, 0, 2, -1, 0, 1] },
  {
    ...
    value: Array(10)
      .fill()
      .map((x) => Math.round(Math.random() * 1)),
  },
];
...
function imageProcess() {
  ...
  image.onload = () => {
    ...
    context.putImageData(
      greyScale(context.getImageData(0, 0, 450, 450), bw),
      0,
      0
    );
  };
...
function processConv() {
  ...
  image.onload = () => {
    ...
    context.putImageData(
      conv_2d(
        context.getImageData(0, 0, 450, 450),
        filterlist[contrast].value,
        1
      ),
      0,
      0
...
function processContrast() {
  ...
  image.onload = () => {
    context.drawImage(image, 0, 0, 450, 450);
    context.putImageData(
      contrast(context.getImageData(0, 0, 450, 450), contra),
      0,
      0
    );
  };
